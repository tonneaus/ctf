...
def login():
    ...
    try:
        ...
        if count > 0: # we want this path
            flask.session["username"] = username
            # ^ modifies session cookie, which is signed so can't forge bc SECRET_KEY is not known to us
...
def monitor():
    if 'username' not in flask.session:
        return flask.redirect('/')
    # ^ can't post here unless we are logged in
